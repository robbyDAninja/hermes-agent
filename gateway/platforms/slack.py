...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Arlo write-approval buttons. Action IDs are
            # arlo_write_approve:<uuid>, arlo_write_deny:<uuid>,
            # arlo_write_approve_thread:<uuid>. The arlo-write skill
            # polls arlo.pending_writes; this handler flips the row.
            @self._app.action(re.compile(r"^arlo_write_(approve|deny|approve_thread):[0-9a-f-]+$"))
            async def handle_arlo_write_action(ack, body, action, client):
                await ack()
    ...
    async def send(
        ...
        try:
            ...
            # ARLO patch #3 — body in arlo/slack_send.py
            from arlo import slack_send
            await slack_send.clear_assistant_status(
                client=self._get_client(chat_id), chat_id=chat_id, thread_ts=thread_ts,
            )
    ...
    async def _handle_arlo_write_action(self, body: dict, action: dict, client) -> None:
        ...
        [Approve] / [Deny] / [Approve all in thread] on an Arlo edit proposal.
        The arlo-write skill polls the row and acts on the new status.
        """
        try:

...
| 1 | ddbcf40e6 | add arlo halt-switch (stop/halt/exit → flag, resume → clear) | halt | extracted-as-shim → arlo/slack_halt.py | When Hermes ships adapter-level lifecycle hooks |
| 2 | 90afced3a | halt also denies in-flight sandbox approvals | halt (folded with #1) | extracted-as-shim → arlo/slack_halt.py | (same as #1) |
